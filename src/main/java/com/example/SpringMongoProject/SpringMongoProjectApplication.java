...
public class SpringMongoProjectApplication {
	...
	public CommandLineRunner runner(UserDataInitializer userDataInitializer,
									Userrepo userrepo, MongoTemplate mongoTemplate) {
		return args -> {
			// Call the initializeData method
			//userDataInitializer.initializeData().run(args);
			userDataInitializer.runner(userrepo, mongoTemplate).run(args);
		};
